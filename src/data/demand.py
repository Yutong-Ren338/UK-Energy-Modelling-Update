...
def demand_era5(resample: str | None = None) -> pd.DataFrame:
    ...
    # demand is in GW
    df = pd.read_csv("data/new/ERA5_full_demand_1979_2018.csv", index_col=0)
    ...
    if resample:
        df = df.set_index("date").resample(resample).mean().reset_index()
    return df
...
def demand_espeni(resample: str | None = None) -> pd.DataFrame:
    ...
    # demand is in MW
    df = pd.read_csv("data/new/espeni.csv", parse_dates=True)
