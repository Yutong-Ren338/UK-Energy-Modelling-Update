...
def fraction_days_without_excess(demand_data: str = "era5", *, return_mean: bool = True) -> pd.Series:
    ...
    # subtract the demand from the renewable generation to get the net power
    supply_df = supply_df.sub(demand_df["demand"], axis=0)

    # count the number of days without excess generation
    days_without_excess = (supply_df < 0).mean(axis=0) if return_mean else (supply_df < 0).sum(axis=0)
